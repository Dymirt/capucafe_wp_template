...
<div class="services  max-md:h-auto  bg-cover   bg-center relative flex justify-center max-sm:p-[5%] max-md:bg-none pb-32">
	...
	<div class="flex relative max-md:flex-col z-10 max-sm:pt-8 max-md:pt-36  pt-48 max-sm:w-full max-md:w-[80%] max-lg:w-[90%]  max-xl:w-[85%] max-2xl:w-[85%]">
		<div class="mssg_parent max-md:w-full w-1/2 max-lg:w-1/2">
			<div id="mssg" class="title text-black w-full max-md:p-10 max-md:justify-center text-5xl  flex max-md:flex-row flex-col mr-[5%]"><span> Capuccino </span> <span class="max-md:ml-2"> Cafe to...</span></div>
		</div>
		<div class="w-full">
...
<script>
	document.addEventListener('DOMContentLoaded', function() {
		...
		(function() {
			let isShopHeader = false;
			let originalHeaderHTML = null;
			...
			window.addEventListener('scroll', () => {
				...
				// Swap to shop header
				if (scrollY > threshold && !isShopHeader) {
					const oldHeader = document.getElementById('menuu');
					if (!oldHeader) return;
				...
				// Swap back to original header
				if (scrollY <= threshold && isShopHeader) {
					const shopHeader = document.querySelector('.header-shop');
					if (!shopHeader || !originalHeaderHTML) return;

					const tempDiv = document.createElement('div');
					tempDiv.innerHTML = originalHeaderHTML;
					const originalHeader = tempDiv.firstElementChild;
