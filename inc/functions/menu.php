...
class Footer_Walker_Nav_Menu extends Walker_Nav_Menu
{
	...
	function start_el(&$output, $item, $depth = 0, $args = [], $id = 0)
	{
		if ($depth === 0) {
			...
			$output .= '<a class="flex-1 h-3.5 justify-start !text-stone-400 text-sm font-bold font-[\'Mulish\'] uppercase !no-underline" href="' . esc_url($item->url) . '">';
			...
			if ($has_children) {
				...
				$output .= '<div class="w-6 h-6  flex justify-center items-center cursor-pointer" onclick="toggleSubmenu(\'submenu-' . $submenu_id . '\')">';
			...
			$output .= '<a class="flex-1 h-3 justify-start font-[\'Mulish\'] uppercase pl-4 !no-underline self-stretch justify-start !text-white text-sm font-normal leading-tight" href="' . esc_url($item->url) . '">';
			$output .= esc_html($item->title);
			$output .= '</a>';
		}
	}
