...
class Footer_Walker_Nav_Menu extends Walker_Nav_Menu
{
	...
	function start_el(&$output, $item, $depth = 0, $args = [], $id = 0)
	{
		if ($depth === 0) {
			...
			$output .= '<a class="justify-start font-[\'Mulish\'] !no-underline self-stretch justify-start !text-white text-sm font-normal leading-tight basis-full" href="' . esc_url($item->url) . '">';
			$output .= esc_html($item->title);
			$output .= '</a>';
		}
	}
