<?php
/*
Template Name: Custom Contact Page
*/

get_header(); ?>

<main id="main" class="site-main">
	<div class="grid grid-cols-1 md:grid-cols-2 px-4">
		<div data-layer="div" class="Div w-full lg:max-w-80 min-w-80 px-5 py-8 bg-stone-50 inline-flex flex-col justify-start items-start gap-3 flex-1 h-full">
			<div data-layer="Title" class="Title w-56 min-w-56 py-3 inline-flex justify-center items-center gap-2.5">
				<div data-layer="Kontakt" class="Kontakt flex-1 justify-start text-stone-700 text-3xl font-normal font-['Didot_LT_Pro']">Kontakt</div>
			</div>
			<div data-layer="Frame 115" class="Frame115 w-60 h-[522px] min-w-60 flex flex-col justify-start items-start h-full">
				<div data-layer="adres" class="Adres self-stretch inline-flex justify-start items-start gap-4">
					<div data-layer="ico" class="Ico self-stretch pt-3 pb-6 inline-flex flex-col justify-start items-start">
						<div data-layer="small" class="Small size-6 relative overflow-hidden">
							<div data-layer="Vector" class="Vector w-4 h-5 left-[3.51px] top-[1.99px] absolute ">
								<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
									<path d="M17.9999 4.48C16.4086 2.8887 14.2504 1.99472 11.9999 1.99472C9.74949 1.99472 7.59123 2.8887 5.99993 4.48C4.40863 6.0713 3.51465 8.22957 3.51465 10.48C3.51465 12.7304 4.40863 14.8887 5.99993 16.48L11.2699 21.76C11.3629 21.8537 11.4735 21.9281 11.5954 21.9789C11.7172 22.0297 11.8479 22.0558 11.9799 22.0558C12.1119 22.0558 12.2426 22.0297 12.3645 21.9789C12.4864 21.9281 12.597 21.8537 12.6899 21.76L17.9999 16.43C19.5846 14.8453 20.4748 12.6961 20.4748 10.455C20.4748 8.21395 19.5846 6.06468 17.9999 4.48ZM16.5699 15L11.9999 19.59L7.42993 15C6.52707 14.0963 5.91241 12.9452 5.66362 11.6923C5.41484 10.4393 5.54312 9.14075 6.03223 7.96068C6.52135 6.78062 7.34935 5.77205 8.41156 5.06248C9.47377 4.35291 10.7225 3.97418 11.9999 3.97418C13.2773 3.97418 14.5261 4.35291 15.5883 5.06248C16.6505 5.77205 17.4785 6.78062 17.9676 7.96068C18.4567 9.14075 18.585 10.4393 18.3362 11.6923C18.0875 12.9452 17.4728 14.0963 16.5699 15ZM8.99993 7.41C8.19264 8.21977 7.73932 9.31657 7.73932 10.46C7.73932 11.6034 8.19264 12.7002 8.99993 13.51C9.59969 14.1108 10.3635 14.5211 11.1956 14.6894C12.0276 14.8577 12.8909 14.7766 13.677 14.4562C14.4631 14.1357 15.1371 13.5903 15.6144 12.8883C16.0917 12.1862 16.3511 11.3589 16.3599 10.51C16.3644 9.94321 16.2553 9.38126 16.0388 8.85739C15.8224 8.33352 15.5032 7.85836 15.0999 7.46C14.7036 7.05458 14.231 6.73154 13.7094 6.50947C13.1877 6.2874 12.6273 6.17068 12.0603 6.16603C11.4934 6.16138 10.9311 6.2689 10.4059 6.48239C9.88067 6.69588 9.40285 7.01113 8.99993 7.41ZM13.6899 12.09C13.311 12.4747 12.8101 12.7159 12.2731 12.7723C11.736 12.8286 11.196 12.6966 10.7454 12.3989C10.2949 12.1012 9.96173 11.6562 9.80294 11.1401C9.64415 10.6239 9.66958 10.0686 9.87489 9.56916C10.0802 9.06967 10.4526 8.65702 10.9285 8.40174C11.4044 8.14646 11.9542 8.06441 12.4839 8.16962C13.0135 8.27483 13.4902 8.56076 13.8324 8.97853C14.1746 9.39631 14.3611 9.91997 14.3599 10.46C14.3454 11.0773 14.0864 11.6635 13.6399 12.09H13.6899Z" fill="#A9957B" />
								</svg>
							</div>
						</div>
					</div>
					<div data-layer="TXT" class="Txt flex-1 pt-3 pb-6 inline-flex flex-col justify-start items-start gap-4">
						<div data-layer="Adresy" class="Adresy self-stretch justify-start text-stone-700 text-base font-bold font-['Mulish'] leading-snug">Adresy</div>
						<div data-layer="Frame 113" class="Frame113 w-full max-w-[480px] flex flex-col justify-start items-start gap-4">
							<div data-layer="Sopot           Aleja Niepodległości 899,          81-861 Sopot" class="SopotAlejaNiepodlegOCi89981861Sopot self-stretch justify-start"><span class="text-zinc-800 text-sm font-bold font-['Mulish'] leading-tight">Sopot </span><span class="text-zinc-800 text-sm font-normal font-['Mulish'] leading-tight">Aleja Niepodległości 899, 81-861 Sopot</span></div>
							<div data-layer="Jastarnia           ks. Sychty 43,           84-140 Jastarnia" class="JastarniaKsSychty4384140Jastarnia self-stretch justify-start"><span class="text-zinc-800 text-sm font-bold font-['Mulish'] leading-tight">Jastarnia </span><span class="text-zinc-800 text-sm font-normal font-['Mulish'] leading-tight">ks. Sychty 43, 84-140 Jastarnia</span></div>
						</div>
					</div>
				</div>
				<div data-layer="tel" class="Tel self-stretch inline-flex justify-start items-start gap-4">
					<div data-layer="ico" class="Ico self-stretch py-6 inline-flex flex-col justify-start items-start">
						<div data-layer="small" class="Small size-6 relative overflow-hidden">
							<div data-layer="Vector" class="Vector size-5 left-[1.98px] top-[2.04px] absolute">
								<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
									<path d="M19.4401 13C19.2201 13 18.9901 12.93 18.7701 12.88C18.3246 12.7818 17.8868 12.6515 17.4601 12.49C16.9962 12.3212 16.4862 12.33 16.0284 12.5146C15.5706 12.6992 15.1972 13.0466 14.9801 13.49L14.7601 13.94C13.7861 13.3982 12.8911 12.7252 12.1001 11.94C11.3149 11.149 10.6419 10.254 10.1001 9.27999L10.5201 8.99999C10.9635 8.78291 11.3109 8.40952 11.4955 7.95168C11.6801 7.49384 11.6889 6.9839 11.5201 6.51999C11.3613 6.09241 11.231 5.65479 11.1301 5.20999C11.0801 4.98999 11.0401 4.75999 11.0101 4.52999C10.8887 3.82561 10.5197 3.18773 9.96972 2.73123C9.41972 2.27473 8.7248 2.0296 8.0101 2.03999H5.0101C4.57913 2.03594 4.15235 2.1248 3.75881 2.30052C3.36527 2.47624 3.01421 2.73469 2.72953 3.05827C2.44485 3.38186 2.23324 3.76298 2.10909 4.1757C1.98494 4.58842 1.95118 5.02305 2.0101 5.44999C2.54284 9.63937 4.45613 13.5319 7.44775 16.5126C10.4394 19.4934 14.3388 21.3925 18.5301 21.91H18.9101C19.6475 21.9111 20.3595 21.6405 20.9101 21.15C21.2265 20.867 21.4792 20.5202 21.6516 20.1323C21.8239 19.7444 21.9121 19.3244 21.9101 18.9V15.9C21.8979 15.2054 21.6449 14.5365 21.1944 14.0077C20.744 13.4788 20.1239 13.1226 19.4401 13ZM19.9401 19C19.9399 19.142 19.9095 19.2823 19.8509 19.4116C19.7923 19.5409 19.7068 19.6563 19.6001 19.75C19.4905 19.8498 19.3601 19.9241 19.2182 19.9673C19.0764 20.0106 18.9268 20.0217 18.7801 20C15.035 19.5198 11.5563 17.8065 8.89282 15.1303C6.2293 12.4541 4.53251 8.96733 4.0701 5.21999C4.05419 5.07351 4.06813 4.92532 4.1111 4.78438C4.15407 4.64344 4.22517 4.51268 4.3201 4.39999C4.41381 4.29332 4.52916 4.20783 4.65848 4.14921C4.7878 4.09058 4.92812 4.06017 5.0701 4.05999H8.0701C8.30265 4.05482 8.52972 4.13087 8.71224 4.27506C8.89476 4.41925 9.02131 4.62256 9.0701 4.84999C9.1101 5.12332 9.1601 5.39332 9.2201 5.65999C9.33562 6.18713 9.48936 6.70517 9.6801 7.20999L8.2801 7.85999C8.1604 7.91491 8.05272 7.99294 7.96326 8.08959C7.87379 8.18623 7.8043 8.2996 7.75877 8.42318C7.71324 8.54677 7.69257 8.67812 7.69795 8.80971C7.70332 8.9413 7.73464 9.07054 7.7901 9.18999C9.2293 12.2727 11.7073 14.7508 14.7901 16.19C15.0336 16.29 15.3066 16.29 15.5501 16.19C15.6748 16.1454 15.7894 16.0764 15.8873 15.9872C15.9851 15.8979 16.0643 15.7901 16.1201 15.67L16.7401 14.27C17.2571 14.4549 17.7847 14.6085 18.3201 14.73C18.5868 14.79 18.8568 14.84 19.1301 14.88C19.3575 14.9288 19.5608 15.0553 19.705 15.2378C19.8492 15.4204 19.9253 15.6474 19.9201 15.88L19.9401 19ZM19.0001 9.99999C19.2653 9.99999 19.5197 9.89463 19.7072 9.7071C19.8947 9.51956 20.0001 9.26521 20.0001 8.99999V4.99999C20.0001 4.73477 19.8947 4.48042 19.7072 4.29288C19.5197 4.10535 19.2653 3.99999 19.0001 3.99999C18.7349 3.99999 18.4805 4.10535 18.293 4.29288C18.1055 4.48042 18.0001 4.73477 18.0001 4.99999V8.99999C18.0001 9.26521 18.1055 9.51956 18.293 9.7071C18.4805 9.89463 18.7349 9.99999 19.0001 9.99999ZM15.0001 9.99999C15.2653 9.99999 15.5197 9.89463 15.7072 9.7071C15.8947 9.51956 16.0001 9.26521 16.0001 8.99999V4.99999C16.0001 4.73477 15.8947 4.48042 15.7072 4.29288C15.5197 4.10535 15.2653 3.99999 15.0001 3.99999C14.7349 3.99999 14.4805 4.10535 14.293 4.29288C14.1055 4.48042 14.0001 4.73477 14.0001 4.99999V8.99999C14.0001 9.26521 14.1055 9.51956 14.293 9.7071C14.4805 9.89463 14.7349 9.99999 15.0001 9.99999Z" fill="#A9957B" />
								</svg>
							</div>
						</div>
					</div>
					<div data-layer="TXT" class="Txt flex-1 py-6 inline-flex flex-col justify-start items-start gap-4">
						<div data-layer="Zadzwoń do nas" class="ZadzwoDoNas self-stretch justify-start text-stone-700 text-base font-bold font-['Mulish'] leading-snug">Zadzwoń do nas</div>
						<div data-layer="Frame 114" class="Frame114 w-full max-w-[480px] flex flex-col justify-start items-start gap-4">
							<div data-layer="Kawiarnie:           Sopot +48 511 554 610          Jastarnia +48 502 770 766" class="KawiarnieSopot48511554610Jastarnia48502770766 self-stretch justify-start"><span class="text-zinc-800 text-sm font-bold font-['Mulish'] leading-tight">Kawiarnie: </span><span class="text-zinc-800 text-sm font-normal font-['Mulish'] leading-tight">Sopot +48 511 554 610 Jastarnia +48 502 770 766</span></div>
							<div data-layer="Wesela:           +48 780 127 222" class="Wesela48780127222 self-stretch justify-start"><span class="text-zinc-800 text-sm font-bold font-['Mulish'] leading-tight">Wesela: </span><span class="text-zinc-800 text-sm font-normal font-['Mulish'] leading-tight">+48 780 127 222</span></div>
						</div>
					</div>
				</div>
				<div data-layer="mail" class="Mail self-stretch border-stone-300 inline-flex justify-start items-start gap-4">
					<div data-layer="ico" class="Ico self-stretch py-6 inline-flex flex-col justify-start items-start">
						<div data-layer="small" class="Small size-6 relative overflow-hidden">
							<div data-layer="Vector" class="Vector w-5 h-4 left-[2px] top-[4px] absolute"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
									<path d="M19 4H5C4.20435 4 3.44129 4.31607 2.87868 4.87868C2.31607 5.44129 2 6.20435 2 7V17C2 17.7956 2.31607 18.5587 2.87868 19.1213C3.44129 19.6839 4.20435 20 5 20H19C19.7956 20 20.5587 19.6839 21.1213 19.1213C21.6839 18.5587 22 17.7956 22 17V7C22 6.20435 21.6839 5.44129 21.1213 4.87868C20.5587 4.31607 19.7956 4 19 4ZM18.59 6L12.71 11.88C12.617 11.9737 12.5064 12.0481 12.3846 12.0989C12.2627 12.1497 12.132 12.1758 12 12.1758C11.868 12.1758 11.7373 12.1497 11.6154 12.0989C11.4936 12.0481 11.383 11.9737 11.29 11.88L5.41 6H18.59ZM20 17C20 17.2652 19.8946 17.5196 19.7071 17.7071C19.5196 17.8946 19.2652 18 19 18H5C4.73478 18 4.48043 17.8946 4.29289 17.7071C4.10536 17.5196 4 17.2652 4 17V7.41L9.88 13.29C10.4425 13.8518 11.205 14.1674 12 14.1674C12.795 14.1674 13.5575 13.8518 14.12 13.29L20 7.41V17Z" fill="#A9957B" />
								</svg>
							</div>
						</div>
					</div>
					<div data-layer="TXT" class="Txt flex-1 py-6 inline-flex flex-col justify-start items-start gap-3">
						<div data-layer="Napisz do nas" class="NapiszDoNas self-stretch justify-start text-stone-700 text-base font-bold font-['Mulish'] leading-snug">Napisz do nas</div>
						<div data-layer="kawiarnia@capuccinocafe.pl" class="KawiarniaCapuccinocafePl self-stretch flex-1 justify-start text-zinc-800 text-sm font-normal font-['Mulish'] leading-tight">kawiarnia@capuccinocafe.pl</div>
					</div>
				</div>
			</div>
		</div>

		<!-- Contact Form Section -->
		<div data-layer="div" class="Div w-full lg:max-w-[490px] min-w-80 px-5 pt-8 pb-20 bg-stone-200 inline-flex flex-col justify-start items-start gap-3 flex-1 h-full">
			<div data-layer="Title" class="Title w-full max-w-[490px] py-3 inline-flex justify-start items-center gap-2.5">
				<div data-layer="Formularz kontaktowy" class="FormularzKontaktowy flex-1 min-w-80 justify-start text-stone-700 text-3xl font-normal font-['Didot_LT_Pro']">Formularz kontaktowy
				</div>
			</div>
			<div data-layer="div" class="Div w-full max-w-[490px] flex flex-col justify-start items-start gap-5">
				<div data-layer="Base" data-property-1="Default" data-property-2="Default" class="Base self-stretch flex flex-col justify-start items-start gap-[5px]">
					<?php echo do_shortcode('[contact-form-7 id="633cabd" title="KONTAKT"]'); ?>

				</div>
			</div>
		</div>
	</div>



	<!-- End of Contact Form Section -->

	<div data-layer="ig" class="Ig self-stretch pt-14 pb-20 inline-flex flex-col justify-start items-center">
		<div data-layer="title" class="Title self-stretch px-10 flex flex-col justify-center items-start gap-2.5">
			<div data-layer="Po więcej zapraszamy     na nasz Instagram" class="PoWiCejZapraszamyNaNaszInstagram self-stretch justify-start text-stone-700 text-3xl font-normal font-['Didot_LT_Pro']">Po więcej zapraszamy na nasz Instagram</div>
			<div data-layer="@capuccinocafe_sopot" class="CapuccinocafeSopot w-60 justify-start text-zinc-800 text-base font-light font-['Mulish'] leading-snug">@capuccinocafe_sopot</div>
		</div>
		<div data-layer="posty" class="Posty self-stretch py-10 inline-flex justify-center items-start gap-5">
			<div data-layer="Frame 43" class="Frame43 w-44 h-64 rounded-sm outline outline-1 outline-offset-[-1px] outline-stone-200 inline-flex flex-col justify-start items-start overflow-hidden">
				<div data-layer="Frame 42" class="Frame42 self-stretch px-4 py-2.5 inline-flex justify-start items-center gap-2.5">
					<img data-layer="image 6" class="Image6 size-6 rounded-[100px]" src="https://placehold.co/24x24" />
					<div data-layer="capuccinocafe_sopot" class="CapuccinocafeSopot justify-start text-zinc-800 text-[10px] font-normal font-['Mulish']">capuccinocafe_sopot</div>
				</div>
				<img data-layer="image" class="Image w-48 h-60" src="https://placehold.co/189x237" />
			</div>
			<div data-layer="Frame 45" class="Frame45 w-44 h-64 rounded-sm outline outline-1 outline-offset-[-1px] outline-stone-200 inline-flex flex-col justify-start items-start overflow-hidden">
				<div data-layer="Frame 42" class="Frame42 self-stretch px-4 py-2.5 inline-flex justify-start items-center gap-2.5">
					<img data-layer="image 6" class="Image6 size-6 rounded-[100px]" src="https://placehold.co/24x24" />
					<div data-layer="capuccinocafe_sopot" class="CapuccinocafeSopot justify-start text-zinc-800 text-[10px] font-normal font-['Mulish']">capuccinocafe_sopot</div>
				</div>
				<img data-layer="image" class="Image self-stretch flex-1" src="https://placehold.co/180x216" />
				<div data-layer="Bounding box" class="BoundingBox size-6 bg-zinc-300"></div>
				<div data-layer="smart_display" class="SmartDisplay w-5 h-4 bg-white"></div>
			</div>
			<div data-layer="Frame 42" class="Frame42 w-44 h-64 rounded-sm outline outline-1 outline-offset-[-1px] outline-stone-200 inline-flex flex-col justify-start items-start overflow-hidden">
				<div data-layer="Frame 42" class="Frame42 self-stretch px-4 py-2.5 inline-flex justify-start items-center gap-2.5">
					<img data-layer="image 6" class="Image6 size-6 rounded-[100px]" src="https://placehold.co/24x24" />
					<div data-layer="capuccinocafe_sopot" class="CapuccinocafeSopot justify-start text-zinc-800 text-[10px] font-normal font-['Mulish']">capuccinocafe_sopot</div>
				</div>
				<img data-layer="image" class="Image self-stretch flex-1" src="https://placehold.co/180x216" />
			</div>
		</div>
		<div data-layer="BTN midle black" data-property-1="Default" class="BtnMidleBlack h-12 px-7 rounded-sm outline outline-1 outline-offset-[-1px] outline-stone-400 inline-flex justify-center items-center gap-2.5">
			<div data-layer="Poznaj nas" class="PoznajNas justify-center text-zinc-800 text-sm font-bold font-['Mulish'] uppercase leading-tight">Dołącz do nas</div>
			<div data-layer="Frame" class="Frame size-6 relative overflow-hidden">
				<div data-layer="Vector" class="Vector w-3.5 h-4 left-[1.56px] top-[10.94px] absolute origin-top-left -rotate-45 bg-stone-400"></div>
			</div>
		</div>
	</div>

</main>

<?php get_footer(); ?>
